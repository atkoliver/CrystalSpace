...
set commands=$verer=1'%~1'; ^
$build=(irm https://api.papermc.io/v2/projects/paper/versions/$ver).builds[-1]; ^
$url="https://api.papermc.io/v2/projects/paper/versions/$ver/builds/$build/downloads/paper-$ver-$build.jar\"; ^
iwr $url -OutFile "./server-$ver/server-paper-$ver.jar"; ^
if (Test-Path -LiteralPath "./server-$ver/server-paper-$ver.jar") ^
{ ^
